...
##########################################################################################
## MODIFIED
from llmtuner.extras.save_and_load import load_valuehead_params, load_valuehead_params2
##########################################################################################
...
def load_model_and_tokenizer(
    ...
    if stage == "rm":
        ###################################################################################################
        ## MODIFIED TO ADD DROPOUT
        model = AutoModelForCausalLMWithValueHeadUQ.from_pretrained(model, summary_dropout_prob=0.2)
        print(model)
        ###################################################################################################
        ...
        if stage == "rm" and model_args.checkpoint_dir is not None: # load valuehead weights to evaluate reward model
            ...
            #######################################################################
            ## MODIFIED FOR WEIGHT LOAD
            if load_valuehead_params2(model, model_args.checkpoint_dir[-1]):
                model.v_head.load_state_dict({
                    "summary.weight": getattr(model, "reward_head_weight"),
                    "summary.bias": getattr(model, "reward_head_bias")
                })
                model.interm_seq.load_state_dict({
                    "lin3.weight": getattr(model, "interm_seq_weight"),
                })
            ################################################################################
...
###################################################################################
## ADDED FOR URM
class AutoModelForCausalLMWithValueHeadUQ(AutoModelForCausalLMWithValueHead):
    def __init__(self, pretrained_model, **kwargs):
        ...
        in_feature_size = self.v_head.summary.in_features
        self.interm_seq = nn.Sequential(OrderedDict([
          ('relu1', nn.ReLU()),
          ('drp2', nn.Dropout(0.2)),
          ('lin3', nn.Linear(in_feature_size, in_feature_size, bias=False)),
          ('relu4', nn.ReLU()),
        ]))
    ...
    def forward(
        ...
        **kwargs,
        ...
        value = self.v_head(self.interm_seq(last_hidden_state)).squeeze(-1)
    ...
    def state_dict(self, *args, **kwargs):
        pretrained_model_state_dict = super().state_dict(*args, **kwargs)
        interm_seq_state_dict = self.interm_seq.state_dict(*args, **kwargs)
        for k, v in interm_seq_state_dict.items():
            pretrained_model_state_dict[f"interm_seq.{k}"] = v
    ##############################################################################
